...
public class Plugin7Adapter extends CustomAdsAdapter {
    ...
    public int getAdNetworkId() {
        return MediationInfo.MEDIATION_ID_38;
    }
    ...
    public void initRewardedVideo(Context activity, Map<String, Object> dataMap, RewardedVideoCallback callback) {
        ...
        if (TextUtils.isEmpty(error)) {
            initSDK(PluginApplication.getInstance());
            if (mDidInitSdk) {
                if (callback != null) {
...
//        intent.setPackage(PluginApplication.getInstance().getPackageName());
//        PluginApplication.getInstance().startService(intent);
    }
